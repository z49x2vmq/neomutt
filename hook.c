...
static void dump_hooks(struct Buffer *buf, struct Hook *h)
{
  ...
  if (h->type & (MUTT_ACCOUNT_HOOK | MUTT_FOLDER_HOOK | MUTT_CRYPT_HOOK | MUTT_MBOX_HOOK |
                  MUTT_APPEND_HOOK | MUTT_CLOSE_HOOK | MUTT_OPEN_HOOK | MUTT_MESSAGE_HOOK |
                  MUTT_REPLY_HOOK | MUTT_SEND_HOOK | MUTT_SEND2_HOOK | MUTT_FCC_HOOK |
                  MUTT_FCC_HOOK | MUTT_SAVE_HOOK | MUTT_SAVE_HOOK))
  {
    struct Buffer *pat = mutt_buffer_pool_get();
    quote_simple(h->regex.pattern, pat);
    mutt_buffer_add_printf(buf, "%s%-80s\t", h->regex.pat_not ? "!" : "", mutt_b2s(pat));
    mutt_buffer_pool_release(&pat);
  }

  //print mailbox (3)
  if (h->type & (MUTT_FCC_HOOK | MUTT_SAVE_HOOK | MUTT_MBOX_HOOK))
    mutt_buffer_add_printf(buf, "%s", mutt_path_escape(mutt_b2s(cmd)));
  ...
  {
    mutt_buffer_add_printf(buf, "%s", mutt_b2s(cmd));
  }
...
/**
 * icmd_bind - Parse 'hooks' commands - Implements Command::parse()
 */
                                    ...
                                    intptr_t data, struct Buffer *err)
{
  ...
  if (mutt_do_pager("hooks", tempfile, MUTT_PAGER_NO_FLAGS, NULL) == -1)
  {
    // L10N: '%s' is the file name of the temporary file
    mutt_buffer_printf(err, _("Could not create temporary file %s"), tempfile);
    return MUTT_CMD_ERROR;
