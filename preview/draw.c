...
#define display(fmt, ...)                                                             \
  ...
  {                                                                                   \
    ...
    int max_byte = mutt_wstr_trunc(mutt_b2s(&disp_buff), mutt_buffer_len(&disp_buff), \
                                   num_cols - col, NULL);                             \
    mutt_window_mvprintw(win, col, row, "%.*s", max_byte, mutt_b2s(&disp_buff));      \
    ++row;                                                                            \
  } while (0)
...

  {
    ...
    const char *data = mutt_b2s(&wdata->buffer);
